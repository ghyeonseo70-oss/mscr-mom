"""
2026-08-19: PROJECT_STATUS.md "1-B" 계획 - _diag_failure_by_phi.py가 뽑아준 실패 파일럿
12개(L_M=0, phi=±90)를 MESH_SIZE_TUBE=0.15(기존 0.3), N_SPLINE_POINTS=80(기존 40)으로
재시도해서 메쉬 세밀화가 실제로 성공률을 올리는지 확인. 증분(inc/initial_inc)은 기존
스윕과 동일하게 유지 - 증분 조정은 이미 폐기된 변수라 여기서 같이 바꾸면 어느 쪽 효과인지
못 가림.

--case-index N으로 케이스 하나만 처리(병렬 실행용, run_pilot_mesh_refinement.sh 참고).
결과는 케이스별 fea_pilot_meshfine_<n>.json으로 저장, --merge로 하나로 합침.
"""
import argparse
import glob
import json
...
MESH_SIZE_TUBE_FINE = 0.15  # 기존 0.3의 절반
N_SPLINE_POINTS_FINE = 80    # 기존 40의 2배

PILOT_CASES = [
    (0.0, 90.0, 40.0), (0.0, 90.0, 50.0), (0.0, 90.0, 60.0), (0.0, 90.0, 70.0),
    (0.0, 90.0, 80.0), (0.0, 90.0, 90.0), (0.0, 90.0, 100.0),
    (0.0, -90.0, 40.0), (0.0, -90.0, 50.0), (0.0, -90.0, 80.0),
    (0.0, -90.0, 90.0), (0.0, -90.0, 100.0),
]
BALL_R = 0.4
PUSH_DEPTH = 0.10
...
def run_one(n):
    ...
    scene.MESH_SIZE_TUBE = MESH_SIZE_TUBE_FINE

    L_M, phi, s = PILOT_CASES[n]
    tag = f"pilot{n:02d}_LM{int(L_M)}_phi{'N' if phi < 0 else 'P'}{int(abs(phi))}_s{int(s)}"
    ...
    subprocess.run(
        [sys.executable, os.path.join(HERE, "get_bent_centerline.py"),
         ...
         "--n_spline_points", str(N_SPLINE_POINTS_FINE)],
        check=True, capture_output=True, text=True,
    )
    inp_name = f"pilot_mesh_{tag}.inp"
    sets_name = f"pilot_sets_{tag}.inp"
    job_name = f"pilotmesh_{tag}"
    t0 = time.time()
    print(f"[{tag}] L_M={L_M}, phi={phi}, s={s} "
          f"(MESH_SIZE_TUBE={scene.MESH_SIZE_TUBE}, N_SPLINE_POINTS={N_SPLINE_POINTS_FINE}) ...", flush=True)
    try:
        ...
        res = rc.run_case(
            ...
            print_tip=True,  # inc/initial_inc 기본값(100/0.01) 그대로 - 기존 스윕과 동일
        )
    ...
    if res is None:
        ...
        row = {"L_M_mm": L_M, "phi_deg": phi, "beta_deg": 0.0, "contact_s_mm": s,
               ...
               "wall_time_s": dt, "success": True, "mesh_refined": True}
        print(f"  [{tag}] [성공] F_mag={res['F_mag_N']*1000:.4f}mN ({dt:.1f}s)", flush=True)

    with open(os.path.join(FEA_DIR, f"fea_pilot_meshfine_{n:02d}.json"), "w", encoding="utf-8") as f:
...
def merge():
    rows = []
    for f in sorted(glob.glob(os.path.join(FEA_DIR, "fea_pilot_meshfine_*.json"))):
        if f.endswith("_all.json"):
            continue
        rows.append(json.load(open(f, encoding="utf-8")))
    out = os.path.join(FEA_DIR, "fea_pilot_meshfine_all.json")
    ...
    print(f"=== 파일럿 결과: {n_success}/{len(rows)} 성공, 평균 {avg_time:.1f}s/케이스 ===")
    print("판정:", "성공률 뚜렷이 상승 -> 확장 재시도 권장" if n_success >= 6
          else "개선 미미 -> 메쉬 세밀화 폐기, L_M 조밀화로 전환 권장")
