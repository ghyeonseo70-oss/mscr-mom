...
L_M/phi를 바꿔가며 여러 형상을 만들 때는 --L_M, --phi, --out으로 CLI 호출
(인자 없이 실행하면 기존과 동일하게 L_M=50, phi=60으로 bent_centerline.json 생성).
"""
import argparse
import json
import os
...
out_path = args.out if args.out else os.path.join(HERE, "bent_centerline.json")
