...
# 2026-08-20 정정: 어제 K_RIGID(3구간, MOM 8mm 완전강체)를 없애고 2구간으로 바꿨었는데,
# 논문 원문(정상 사본, C:\...\김현서\김현서\Workspace_Expansion_...pdf)을 직접 읽어서 확인해보니
# 이게 잘못된 변경이었음이 드러남 - 논문 식(9) 바로 아래: "we treat the length of the magnet
# as a straight line and locate it into the coordinates (xLM, yLM)" - 논문 자체가 MOM
# 길이(HM)를 안 휘는 직선 구간으로 명시적으로 취급함(식 7,8의 구간 길이가 LM-HM/2,
# L-LM-HM/2로 합쳐서 L보다 HM만큼 모자란 게 바로 이 강체구간). "FEA 메쉬가 균일재질"이라는
# 근거로 2구간으로 바꿨던 건, FEA 메쉬 재질(접촉 시뮬레이션용)과 자기토크 형상계산(해석모델)을
# 착각해서 섞은 것 - 서로 다른 층위라 모순되지 않음. 3구간(K_RIGID)으로 되돌림.
E_YOUNG = 850 * 1000  # Pa
...
def solve_shape(L_M, phi_deg, loads=None, s_t=None, Fx=0.0, Fy=0.0, return_curve=False,
    ...
    def shoot(theta_L_guess, collect_curve=False):
        ...
        for i in range(len(breakpoints) - 1, 0, -1):
            ...
            # 논문 식(9),(10) 그대로: MOM 길이(H_M)는 안 휘는 직선 구간으로 취급하므로
            # 여기서 H_M_m/2만큼 cos/sin으로 직접 밀어줌(적분이 아니라 기하로 처리).
