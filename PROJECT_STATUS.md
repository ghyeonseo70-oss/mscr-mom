...
푸시됨. **와이어 스윕 결과는 버그로 인해 사실상 무효**(`*EMBEDDED ELEMENT`가 작동 안 해서
와이어가 물리적으로 아무 영향도 못 줌 - 아래 5번 마지막 문단 참고) - 재구현 후 다시 돌려야 함.
...
**⚠️ 원인 확정(2026-08-19): (b)였음 — `*EMBEDDED ELEMENT`가 전혀 작동 안 하고 있었음.**
`*NODE PRINT, NSET=N_WIRE_BEAM`으로 `.dat`에 와이어 절점 변위를 직접 찍어서 확인(위 (a)/(b)
판별을 위해 141개 와이어 절점 전부): **전부 정확히 (0, 0, 0)** — 고정단 근처뿐 아니라 접촉점
바로 근처(s=15mm, 하중이 가장 크게 걸리는 위치) 절점까지도 전혀 안 움직임. 즉 와이어가 튜브
변형과 완전히 분리된(mechanically disconnected) 채로 풀리고 있었음 - `*EMBEDDED ELEMENT`가
이 CalculiX 2.17(2020년 빌드)에서 실제로 구속을 안 걸었거나 무시된 것으로 보임(카드 자체는
`.inp`에 정상적으로 들어가 있음, 문법 오류로 보이진 않음 - CalculiX가 에러 없이 그냥
무시했을 가능성). **이번 세션에서 만든 와이어 FEA 결과(`fea_matv2wire_all.json`)는 사실상
"와이어 없음"과 동일한 데이터라 학습 등에 쓰면 안 됨** - 와이어를 실제로 반영하려면
`*EMBEDDED ELEMENT` 대신 다른 구속 방식(`*TIE`(면 기반, 와이어가 가늘어서 표면 정의가
까다로움) 또는 명시적 `*EQUATION`/MPC로 각 와이어 절점을 가장 가까운 솔리드 절점(들)에
직접 묶는 방식)으로 재구현 필요 - 다음 세션 작업.
