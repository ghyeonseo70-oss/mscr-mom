...
푸시됨. 와이어 효과가 예상보다 훨씬 작게 나와서(거의 0%) 원인 확인이 필요함 — 아래 5번 마지막
문단 참고.
...
### 5) 다음 할 일 — 재학습
`train_segment_classifier_singleprobe_beta0180_4seg.py`의 `SOURCES`를
`fea_lm_phi_pos_matv2_all.json`(+ beta=180 부호반전 보강분, `check_beta180_symmetry.py`가
대칭성 확인해줬으니 beta=0 데이터를 부호반전해서 beta=180 표본 수 보강 권장)으로 바꾸고
대체모델(surrogate) 재학습 → 15만개 합성데이터 재생성 → CNN 재학습(150k, GPU ~110분) 진행하면
됨. 재학습 후 성능이 위 91.0%/R²=0.938과 다르면 — 상수가 바뀌었으니(BR 0.36→0.4T, K2
9.89e-7→6.26e-7) 절대 수치는 달라질 수 있음, 큰 폭 저하가 아니면 정상.

**진행 전 참고**: 아래 "K1 구간 니티놀 와이어 FEA 실험" 결과가 아직 원인 미확정 상태(와이어
넣어도 접촉힘 거의 안 변함, 예상과 다름) — 이게 K1=2×K2 가정 자체를 의심하게 만드는 결과라,
재학습을 지금 바로 할지 이 의문부터 해소하고 할지는 사용자 판단 필요.
...
**⚠️ 검증 결과(2026-08-19 스윕 완료 후 갱신) — 효과가 사실상 없는 것으로 나옴, 원인 미확정**:
...
**원인 미확정** — 다음 세션에서 확인 필요: (a) 진짜 물리적으로 그 위치·이 정도 깊이(0.1mm
압입)에서는 국소 변형장이 벽 안쪽 0.75mm까지 별로 안 미치는 것일 수도 있고, (b) CalculiX
2.17(2020년 빌드)의 `*EMBEDDED ELEMENT`가 이 상황에서 실제로 안 걸리고 있을 수도 있음(직접
`.frd`에서 빔 절점 변위를 뽑아 근처 튜브벽 절점과 비교해보려 했으나 형식 문제로 명확히 확인
못 함 - 다음 세션에서 `-1  <node_id>` 라인의 정확한 컬럼 폭을 다시 확인해서 재시도 권장, 또는
`*NODE PRINT, NSET=WIRE_BEAM_NODES` 같은 걸 추가해서 `.dat`에 직접 뽑는 게 더 안전함). (a)가
맞다면 이 정도로 얇은(100um) 와이어는 K1 구간 FEA 접촉힘 결과에 실질적 영향이 없다는 게 이번
실험의 정직한 결론이고, (b)면 임베드 자체를 다른 방식(예: `*TIE` 또는 명시적 MPC)으로
재구현해야 함.
