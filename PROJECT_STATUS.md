...
**분류/회귀 모델 성능** (검증 15,000개 기준):

| 모델 | 구간분류 balanced acc | 연속값 s (R², MAE) | Fx (R²) | Fy (R²) | F_mag (R²) | L_M/phi (R²) |
|---|---|---|---|---|---|---|
| multi-probe(11) + 5구간 (2026-08-11, `..._multiprobe_auxreg.py`, 옛 물리상수) | 86.0% | 0.954, 4.37mm | 0.90 | 0.88 | 0.92 | - |
| single-probe(1) + 4구간 + beta0180, 옛 물리상수 (`..._singleprobe_beta0180_4seg.py`) | 91.0% | 0.938, 3.06mm | 0.943 | 0.974 | 0.971 | 0.976 / 0.975 |
| ↑ 동일 스크립트, **새 물리상수로 재학습 완료(2026-08-19, 이 세션)** — **현재 최신/최고** | **91.8%** | **0.961, 2.71mm** | 0.948 | 0.936 | 0.939 | 0.968 / 0.944 |

**재학습 결과 요약**: 구간분류/연속값 s는 오히려 개선(91.0%→91.8%, R²0.938→0.961), 힘/형상
회귀는 소폭 하락하지만 전부 R²>0.93 유지 — 새 물리상수(BR/K1/K2)로 바꿔도 전체적인 파이프라인
성능은 유지됨. FEA 소스 데이터가 664개→181개로 줄었는데도(대체모델 5-fold R² 일부 저하:
tip_uz R²=-1.16, Fy/Fz도 약함 — 181개로는 부족한 타겟들) 최종 분류기 성능이 유지된 건 15만개
합성데이터의 노이즈 평균화 효과로 보임. 모델: `models/position_segment_classifier_singleprobe_beta0180_4seg.pth`
(재학습 로그: `현서/scripts/contact_scenarios/fea/run_singleprobe_beta0180_4seg_matv2.log`).
GPU(RTX 5080) 있는 컴퓨터에서 총 24.3분 소요(대체모델 162s + 합성데이터 생성 1197s + CNN
학습 101s).
...
**재학습**: ✅ 1차 완료(위 표의 91.8%) — 그런데 **2026-08-19(같은 날) 늦게, 비판적 리뷰를 거쳐
이 91.8%가 측정된 방식 자체에 결함이 있다는 게 드러남**(순환검증: 서로게이트가 만든 합성데이터
안에서만 검증했음). 파이프라인을 5군데 고쳤고(아래 참고), **91.8%는 폐기하지 말고 "구검증
방식 기준" 참고치로만 남겨두되, 고친 스크립트로 재실행해서 새 숫자를 받아야 함** — 이게
다음 컴퓨터(GPU)가 할 일.

### [2026-08-19 늦게, 최우선] 비판적 리뷰로 찾은 파이프라인 결함 5가지 — 수정 완료, 재실행 필요
...
**실행 방법** (GPU 컴퓨터, `현서/scripts/contact_scenarios/fea/`에서):
```bash
N_SAMPLES=150000 N_EPOCHS=60 python train_segment_classifier_singleprobe_beta0180_4seg.py
```
출력 맨 끝 "=== 순수 실측 FEA 검증 ===" 섹션의 숫자가 **가장 신뢰할 수 있는 성능 지표**임 -
합성-val 숫자(91.8%에 대응하는 값)와 이 실측 숫자가 크게 벌어지면(예: 실측 balanced acc가
합성-val보다 10%p 이상 낮으면) 그게 바로 지금까지 우려했던 "서로게이트 환각 학습" 문제가
실제로 있었다는 증거임 - 벌어지는 정도를 꼭 확인하고 보고할 것. n=36이라 이것도 완벽한
통계는 아니지만(신뢰구간이 넓음), 지금까지의 n=3보다는 훨씬 나은 신호임.
