...
**분류/회귀 모델 성능** (검증 15,000개 기준, 둘 다 아직 **옛 물리상수**로 학습한 결과 —
재학습 전):

| 모델 | 구간분류 balanced acc | 연속값 s (R², MAE) | Fx (R²) | Fy (R²) | F_mag (R²) | L_M/phi (R²) |
|---|---|---|---|---|---|---|
| multi-probe(11) + 5구간 (2026-08-11, `..._multiprobe_auxreg.py`) | 86.0% | 0.954, 4.37mm | 0.90 | 0.88 | 0.92 | - |
| single-probe(1) + 4구간 + beta0180 (`..._singleprobe_beta0180_4seg.py`, **현재 최신/최고**) | **91.0%** | **0.938, 3.06mm** | 0.943 | 0.974 | 0.971 | 0.976 / 0.975 |
...
**재학습 준비 상태**: SOURCES 교체 완료(`fea_lm_phi_pos_matv2_all.json` 1개, 부호반전 증강
불필요 — 실측 beta=180 데이터가 이미 있음), **아직 실행은 안 함** — 이 컴퓨터가 CPU-only라
GPU 컴퓨터에서 돌리는 걸 권장(아래 5번 참고).

## ⚠️ 2026-08-18~19 세션 업데이트 (CalculiX 있는 컴퓨터) — 스윕 완료, 재학습 준비 완료(실행 대기)
...
### 5) [2026-08-19 완료] 재학습 준비 — SOURCES 교체 완료, 실행만 하면 됨
...
**남은 건 실행뿐**: 대체모델(surrogate) 재학습 → 15만개 합성데이터 재생성 → CNN 재학습(150k)
순서로 돌리면 됨. **⚠️ 이 컴퓨터는 CPU-only(torch 2.12.1+cpu, `torch.cuda.is_available()=False`)
라 150k 학습이 옛 기록(GPU ~110분)보다 훨씬 오래 걸릴 것으로 예상됨** — GPU 있는 컴퓨터에서
돌리는 걸 권장. 재학습 후 성능이 위 91.0%/R²=0.938과 다르면 — 상수가 바뀌었으니(BR 0.36→0.4T,
K2 9.89e-7→6.26e-7) 절대 수치는 달라질 수 있음, 큰 폭 저하가 아니면 정상.
