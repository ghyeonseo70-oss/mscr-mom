...
- 니티놀 와이어 FEA 실험(K1=2×K2 가정 검증 시도): 사용자 실측 스펙(지름100um, E=28GPa)으로
  ...
  0) — **원인 미확정**(물리적으로 실제 그런 건지 FEA 임베드 설정 문제인지 다음 세션 확인
  필요). EN_FACTOR=2.0의 근거가 흔들리는 결과.
...
푸시됨. 와이어 효과가 예상보다 훨씬 작게 나와서(거의 0%) 원인 확인이 필요함(아래 6번).
누락됐던 재학습 스크립트(`..._singleprobe_beta0180_4seg.py`) push 완료, SOURCES도 새 FEA
데이터로 교체 완료 — 재학습 실행만 남음(아래 5번).
...
**[2026-08-19 원인 규명] (a)가 맞는 것으로 결론** — 논문 Section II(설계)에 "윤활을 위해
30cs 실리콘 오일을 넣는다(to facilitate lubrication, 30 cs of silicone oil)"는 내용이 있음
(사용자 확인). 즉 와이어(그리고 MOM 캐리지)는 튜브 벽에 접착된 보강재가 아니라 **오일이 채워진
내강 속에서 거의 마찰 없이 미끄러지는 별개 부품**임 - 텐던이 MOM 위치를 옮기려면 애초에
슬라이드가 가능해야 하니 설계 목적과도 부합함. 이렇게 헐겁게(접착 없이) 떠 있는 부품은 외부
접촉에 의한 국소 굽힘에 거의 저항을 안 보태는 게 물리적으로 정상 — **FEA의 "거의 0%" 결과가
버그(위 (b))가 아니라 맞는 답이었을 가능성이 높음.** 참고로 "와이어가 벽에 완전 접착"이라고
가정하고 평행축 정리로 계산해보면 오히려 그 방향으로 튜브 자체보다 **~200배** 뻣뻣해진다는
비현실적인 값이 나와서(부록 계산: E_nitinol=28GPa, r=50um, 오프셋 0.75mm 기준), "접착
안 됨"이 정성적으로도 훨씬 그럴듯함.

**결론**: `include_wire=False`(와이어 미포함) 기본 파이프라인이 원래 맞는 근사였음 — 재학습을
와이어 포함 버전으로 다시 할 필요 없음, 이미 진행 중인/향후 재학습 그대로 유효.

**여전히 안 풀린 것**: 그럼 교수님 MATLAB의 `EN=2`(K1이 K2보다 2배 뻣뻣하다는 가정)는 와이어
보강 때문이 아닐 가능성이 높아짐 - 진짜 근거가 뭔지는 미확인(다른 구조적 차이일 수도, 그냥
근사값일 수도). 급한 사안은 아니라 실측/교수님 문의 시 확인할 항목으로만 남겨둠.
